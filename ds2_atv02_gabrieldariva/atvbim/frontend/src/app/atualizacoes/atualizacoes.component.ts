...
import { AtualizacoesService, AtualizacoesEntity } from '../_services/atualizacoes.services';
...
@Component({
  ...
  styleUrls: ['./atualizacoes.component.scss']
})
export class AtualizacoesComponent implements OnInit {

  public displayedColumns: string[] = ['versao', 'conteudo', 'options'];
  public atualizacoess: AtualizacoesEntity[] = [];
  public zonas: UsuarioEntity[] = [];

  public dataSource = new MatTableDataSource<AtualizacoesEntity>();

  public errorMessage: string;
  public loading: boolean;

  public atualizacoes: AtualizacoesEntity = new AtualizacoesEntity();

  @ViewChild(MatSidenav, {static: true}) sidenav: MatSidenav;

  constructor(private service: AtualizacoesService, private snackBar: MatSnackBar,
  ...
  ngOnInit(): void {
    ...
    //Carrega a lista de atualizacoess
    this.service.listarTodos().subscribe(result => {
      
      //Alimenta o datasource da tabela com a lista recebido da service
      this.atualizacoess = result as [];

      //Alimenta o datasource com os atualizacoess
      this.dataSource.data = this.atualizacoess;
    ...
    //Listner do evento createAtualizacoes
    this.socketClient.fromEvent('createAtualizacoes').subscribe(result => {
      this.atualizacoess.push(result as AtualizacoesEntity)
      this.dataSource.data = this.atualizacoess;
    })

    //Listner do evento deleteAtualizacoes
    this.socketClient.fromEvent('deleteAtualizacoes').subscribe(result => {
      let atualizacoes = result as AtualizacoesEntity;
      let index = this.atualizacoess.findIndex( item => item.id == atualizacoes.id);

      this.atualizacoess.splice(index, 1);

      this.dataSource.data = this.atualizacoess;
    })

    //Listner do evento createAtualizacoes
    this.socketClient.fromEvent('updateAtualizacoes').subscribe(result => {
      let atualizacoes = result as AtualizacoesEntity;
      let index = this.atualizacoess.findIndex( item => item.id == atualizacoes.id);

      this.atualizacoess[index] = atualizacoes;
  ...
  public confirmar(): void {
    ...
    this.service.salvar(this.atualizacoes).subscribe(result => {
   ...
   * @param atualizacoes 
   */
  public excluir(atualizacoes: AtualizacoesEntity): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.service.excluir(atualizacoes.id).subscribe(result => {
  ...
  public adicionar(): void {
    ...
    this.openSidenav(new AtualizacoesEntity());
  }
   ...
   * @param atualizacoes
   */
  public editar(atualizacoes: AtualizacoesEntity): void {
    //Como atualizacoes é passado um objeto da tabela por referencia, 
    //se não for feito uma copia deste, ao alterar a linha da 
    //tabela altera junto.
    this.openSidenav(Object.assign({}, atualizacoes));
  }
   ...
   * @param atualizacoes 
   */
  private openSidenav(atualizacoes: AtualizacoesEntity): void {
